...
int Search_Transposition(Array *arr,int key)
{
    int index=-1;
    for(int i=0;i<arr->length;i++)
    {
...
int Search_MoveToFront(Array *arr,int key)
{
    int index=-1;
    for(int i=0;i<arr->length;i++)
    {
...
void main(){
  ...
  scanf("%d",&key);
  int index=Search_Transposition(arr,key);
  if(index!=-1)
  ...
  Display(arr);
  index=Search_MoveToFront(arr,key);
  if(index!=-1)
    printf("%d found at index %d",key,index);
  else
  {
